...
- 🗂 **产物侧边栏**：实时列出代理通过 `write` / `edit` 创建或修改的文件；列表与预览区之间的分界线可拖动调整。
...
- ⚙️ **设置面板**：DSH 设置里新增「Popout Sidebar」选项卡，可开关自动刷新、设置最短面板宽度（存于 `localStorage`）；面板更宽可通过拖动左边缘调整。
...
| 最短面板宽度 | 30% | 面板最小宽度（占窗口宽度的百分比，20–60%）；更宽可拖动面板左边缘调整 |
