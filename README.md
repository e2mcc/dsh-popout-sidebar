...
- 🌳 **文件树**：侧边栏与独立标签页内都有「文件树」标签页，可浏览工作区目录（懒加载展开，点击文件即预览）。
...
- ⚙️ **设置面板**：在 DSH 设置里新增「单页侧卡」选项卡，可开关自动刷新、设置最短面板宽度（存于 `localStorage`）；面板更宽可通过拖动左边缘调整。
...
在 DSH 设置面板（左下角 ⚙️）里会多出一个「**单页侧卡**」选项卡，包含以下设置：
